...
# %% md
"""
英雄联盟（LoL）是美国游戏开发商RiotGames（2011年由腾讯收购）开发和发行的一款多人在线战斗竞技游戏。
...
3.
Elemental Drakes / Elder Dragon（小龙 / 远古龙）
4.
Rift Herald（峡谷先锋）
5.
Baron Nasho（纳什大龙）
...
Time：创建时间
- Game
Duration( in seconds)：游戏持续时间（秒）
- Season
ID：赛季ID
- Winner(1 = team1, 2 = team2)：获胜队伍
- First
Baron, dragon, tower, blood, inhibitor and Rift
Herald(1 = team1, 2 = team2, 0 = none)：第一条纳什大龙，小龙，塔，一血，水晶，峡谷先锋
- Champions and summoner
spells
for each team(Stored as Riot's champion and summoner spell IDs)：每只队伍选择的英雄和召唤术
...
#数据集一共有51490条记录，其中红方获胜的次数为26077次，占比50.6 % ，蓝方获胜的次数为25413次，占比49.4 % 。不存在样本不平衡的情况。
...
# %% md
"""
结论：获得一血的队伍胜率相对较高，在第一队的比赛中，首先获得一血时的胜率为59.48 %，相较未获得一血的比赛高18 %。
在第二队的比赛中，获得一血时的胜率为58.72 %，相较未获得一血的比赛高18 %。
"""
# %% md
...
# %% md
"""
结论：从数据来看，第一个防御塔看起来是比较有说服力的指标。在第一队的比赛中，首先摧毁一塔时队伍的胜率高达70.84 %，相较未获得一塔的比赛高41.64 %。
在第二队的比赛中，有相近的数据表现。
"""
# %% md
...
plot_bar_horizontal(input_col='firstDragon', target_col='winner', title_name='击杀第一条小龙对获胜的影响')
...
# %% md
"""
结论：选择第一个队伍的摧毁防御塔数作为影响因素，可以看出，摧毁的防御塔数量越多，获胜的概率越大。当数量大于8个时，胜率大于85 %。
11个防御塔全部摧毁时的胜率为99.16 %，当然也有8.4‰的翻盘概率。
"""
# %% md
...
plot_bar_vertical(input_col='t1_dragonKills', target_col='winner', title_name='击杀小龙数对获胜影响')
...
第一队拿了第一血，第一塔，第一大龙，第一条小龙和第一峡谷先锋，而第二队只拿了第一个水晶。

第一队的塔，水晶，大龙和小龙杀死的数量分别是10, 2, 1, 4
和塔，水晶，大龙和小龙的数量分别是7, 2, 1, 1。
"""
# %%

# 新数据
# 一血 一塔 水晶 大龙 小龙 峡谷先锋（归属哪一方）
#  1    1   2   1   1    1
# 红方推塔数量 推水晶数量 大龙数量 小龙数量
#     10         2       1      4
# 蓝方推塔数量 推水晶数量 大龙数量 小龙数量
#     7          2       1      1
